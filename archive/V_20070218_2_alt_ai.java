	...
	private V()	{
		...
		while( true ) {
			...
			if ( _state == AI_INPUT_STATE ) {
				...
				int[] dx = new int[_ai_level];
				int[] dy = new int[_ai_level];
				for ( int i = 0; i < _ai_level; i++ ) {
					dx[i] = -1;
					dy[i] = -1;
				}
				boolean hasNextPath = true;
				
				while( hasNextPath ) {
					// initial values for path
					int x0 = p_x[_currentPlayer];
					int y0 = p_y[_currentPlayer];
					int u0 = p_u[_currentPlayer];
					int v0 = p_v[_currentPlayer];
					boolean goOn = true;
					int i = 0;
					...
					for ( ; i < _ai_level; i++ ) {
						...
						// variables
						int u = u0+dx[i]*GRID_SIZE;
						int v = v0+dy[i]*GRID_SIZE;
						int pixs = Math.max( Math.abs( u ), Math.abs( v ) );

						// check pixels for move; skip first
						for ( int p = 1; goOn && p <= pixs; p++ ) {
							x = x0 + Math.round( p*u/(float)pixs );
							y = y0 + Math.round( p*v/(float)pixs );
							...
							if ( tv == START_FINISH ) { // start/finish
								if ( _p_halfway[_currentPlayer] ) { // finish reached
									...
									int f = i*100-1000-pixs+random( -1,1 );
									if ( f < _tv ) {
										_tv = f;
										_du = dx[0]*GRID_SIZE;
										_dv = dy[0]*GRID_SIZE;
				        ...
				        if ( !goOn ) { 
							// fast forward next levels
							for ( int ii = i+1; ii < _ai_level; ii++ ) {
								dx[ii] = 1;
								dy[ii] = 1;
							}		        	
				        	break; 
				        ...
				        // set initial values for next level
				        x0 = x;
				        y0 = y;
				        u0 = u;
				        v0 = v;
