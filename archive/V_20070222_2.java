...
class V extends JFrame
{
	...
	private static boolean _ai_halfway = false;
	...
	private V()	{
        ...
        //vr.enableEvents( KeyEvent.KEY_PRESSED );
        //vr.setVisible( true );
        show();
		...
		Graphics g_bf = strategy.getDrawGraphics();
		Graphics g_bg = bg.getGraphics();
		Graphics2D g_tr = (Graphics2D)track.getGraphics();
		Graphics g_ol = null;
		...
		// ##################################################
		// background
		g_bg.setColor( new Color( 0,0,0 ) );
		for ( int x = 0; x < WIDTH; x++ ) {
			for ( int y = 0; y < HEIGHT; y++ ) {
				g_bg.setColor( new Color( random(190,210),random(190,210),random(140,160) ) );
				g_bg.fillRect( x,y,1,1 );							
			}
		}
		
		// menu background
		g_bg.setColor( new Color( 0,0,0,150 ) );
		g_bg.fillRect( TRACK_WIDTH,0,MENU_WIDTH,HEIGHT );			
		...
		while( true ) {
			...
			if ( state == NEW_TRACK_STATE ) {
				...
				g_tr.drawImage( bg, 0, 0, null );

				// draw lines/curbs
				g_tr.setColor( new Color( 254,254,254 ) );
				g_tr.setStroke( new BasicStroke( TRACK_SIZE+4,BasicStroke.CAP_BUTT,BasicStroke.JOIN_ROUND ) );
				g_tr.drawPolygon( x,y,N );
				
				final float[] DASH = { 6 };
				g_tr.setColor( new Color( 254,0,0 ) );
				g_tr.setStroke( new BasicStroke(
					TRACK_SIZE+4,
				...
				g_tr.setStroke( new BasicStroke(
		                ...
		                0 ) );
				for ( int i = 0; i < N; i++ ) {
					g_tr.drawOval( x[i]-(TRACK_SIZE+2)/2,y[i]-(TRACK_SIZE+2)/2,TRACK_SIZE+2,TRACK_SIZE+2 );
				}
				...
				// draw track
				g_tr.setColor( TRACK_COLOR );
				g_tr.setStroke( new BasicStroke( TRACK_SIZE,BasicStroke.CAP_ROUND,BasicStroke.JOIN_ROUND ) );
				g_tr.drawPolygon( x,y,N );
				
				// draw start/finish
				final int[] sfx = { SF_X,SF_X-2*GRID_SIZE,SF_X-2*GRID_SIZE };
				final int[] sfy = { SF_Y,SF_Y+TRACK_SIZE/2+1,SF_Y-TRACK_SIZE/2-1 };
				g_tr.setColor( SF_COLOR );
				g_tr.setStroke( new BasicStroke() );
				g_tr.fillPolygon( sfx,sfy,3 );
				...
				for ( int xx = 0; xx < TRACK_WIDTH; xx++ ) {
					for ( int yy = 0; yy < HEIGHT; yy++ ) {
						...
						if ( color.equals( SF_COLOR ) ) { // start/finish
							...
							g_tr.setColor( new Color( c,c,c ) );
							g_tr.fillRect( xx,yy,1,1 );
							if ( (double)xx%(double)GRID_SIZE == 0.0 || (double)yy%(double)GRID_SIZE == 0.0 ) {
								g_tr.setColor( GRID_COLOR );
								g_tr.fillRect( xx,yy,1,1 );
			                }
						}
					...
					for( int i = 0; i < MAX_PLAYERS; i++ ) {
						if ( p_x[i] != 0 ) {
							...
							while( time0 + 1000.0*(p_i[i]/(double)pixs) < time ) {
								...
								g_ol.setColor( new Color( 
									PLAYER_COLORS[i].getRed(),
									PLAYER_COLORS[i].getGreen(),
									PLAYER_COLORS[i].getBlue(),
									10 ) );
								g_ol.fillOval( p_x[i]-PLAYER_R, p_y[i]-PLAYER_R, 2*PLAYER_R, 2*PLAYER_R );
								...
								// check surface
								g_ol.setColor( PLAYER_COLORS[i] );
								if ( v == WALL ) { // wall
									g_ol.drawString( "player " + String.valueOf( i+1 ) + " crashed", TRACK_WIDTH+10, message_pos );
									message_pos += 15;
									p_x[i] = 0; // out of game
								...
								if ( v == START_FINISH && p_halfway[i] ) { // start/finish && finished
									...
									g_ol.drawString( "player " + String.valueOf( i+1 ) + ": " + String.valueOf( dtime ) , TRACK_WIDTH+10, message_pos );
									message_pos += 15;
			...
			if ( state == AI_INPUT_STATE ) {
				...
				_ai_halfway = p_halfway[currentPlayer];
				runAI( 
					p_x[currentPlayer], 
					p_y[currentPlayer],
					p_u[currentPlayer], 
			...
			// draw track
			g_bf.drawImage( track, 0, 0, null );
			
			// draw overlay
			g_bf.drawImage( overlay, 0, 0, null );

			// draw main menu
			g_bf.translate( TRACK_WIDTH,0 );
			if ( state == MENU_STATE || state == NEW_TRACK_STATE ) {
				g_bf.setColor( PLAYER_COLORS[MAX_PLAYERS-1] );
				g_bf.drawOval( 15,menu-8,8,8 );
				g_bf.drawString( "race",28,MENU_RACE );
				g_bf.drawString( "players: " + String.valueOf( h_n ),28,MENU_PLAYERS );
				g_bf.drawString( "ais: " + String.valueOf( ai_n ),28,MENU_AIS );
				g_bf.drawString( "ai level: " + String.valueOf( (ai_level-1) ),28,MENU_AI_LEVEL );
				g_bf.drawString( "new track",28,MENU_NEW_TRACK );
			}
			else {
				// draw time
				g_bf.setColor( PLAYER_COLORS[MAX_PLAYERS-1] );
				g_bf.drawString( "time: " + String.valueOf( time/1000.0 ),20,30 );
			}
			
			if ( best < 1000 ) {
				// draw best time
				g_bf.setColor( PLAYER_COLORS[MAX_PLAYERS-1] );
				g_bf.drawString( "best: " + String.valueOf( best ),10,150 );
			}
			g_bf.translate( -TRACK_WIDTH,0 );
			...
			if ( state != MENU_STATE ) {
				for ( int i = 0; i < MAX_PLAYERS; i++ ) {
					if ( p_x[i] != 0 ) {
						g_bf.setColor( PLAYER_COLORS[i] );
						g_bf.drawOval( p_x[i]-PLAYER_R,p_y[i]-PLAYER_R,2*PLAYER_R,2*PLAYER_R );
					}
				}
			}
			...
			if ( state == INPUT_STATE ) {
				g_bf.setColor( PLAYER_COLORS[currentPlayer] );
				g_bf.drawString( "player: " + String.valueOf( currentPlayer+1 ),TRACK_WIDTH+20,50 );
				g_bf.fillOval( p_x[currentPlayer]-PLAYER_R-1,p_y[currentPlayer]-PLAYER_R-1,2*PLAYER_R+2,2*PLAYER_R+2 );
				...
				for ( int dx = -1; dx < 2; dx++ ) {
					for ( int dy = -1; dy < 2; dy++ ) {
						...
						g_bf.drawOval( x-PLAYER_R,y-PLAYER_R,2*PLAYER_R,2*PLAYER_R );
					}
				}
			}
	...
	private void runAI( int x0, int y0, int u0, int v0, int du0, int dv0, int n,
			int ai_level, int[][] trace, int halfway_tv ) {
		final int GRID_SIZE = 15;
		final int START_FINISH = -1;
		final int WALL = -2;
			...
			for ( int dv = -GRID_SIZE; dv < 2*GRID_SIZE; dv += GRID_SIZE ) {
				...
				if ( goOn ) {
					if ( n > 0 ) { // next iteration level
						runAI( x,y,u,v,du0,dv0,n,ai_level,trace,halfway_tv );
					}
